...
const PortFolioImage = ({ title, src }: PortFolioImage) => {
    ...
    function onBtnClick(num: number) {
        if (imgRef.current) {
            ...
            if (num === 4) {
                if (title === 'poke') imgRef.current.src = srcObj['poke4'];
                if (title === 'shopping') imgRef.current.src = srcObj['shopping4'];
    ...
    return (
        <div className={styles.portFolioImage}>
            ...
            <div className={styles.btns} ref={btnRef}>
                ...
                { (title === 'poke' || title === 'shopping') && <div onClick={() => onBtnClick(4)}></div>}
            </div>
        </div>
    )
}
